...
def load_frame(webcam_url):
    """Opens the webcam stream using the given URL and returns a single frame."""
...
def capture_frames(webcam_url, duration, save_folder):
    """
    Captures raw frames from the webcam for the specified duration.
    Saves each frame locally in save_folder.
    Returns a list of saved frame filenames.
    """
    ...
    while (time.time() - start_time) < duration:
        ...
        frame = load_frame(webcam_url)
        filename = save_frame(frame, save_folder)
        filenames.append(filename)
        #time.sleep(0.25)  # Capture a frame every 0.25 second
    return filenames
...
def create_gif(filenames, gif_name="demo_output.gif", frame_duration=1.0):
    """
    ...
    imageio.mimsave(gif_name, images, fps=2, loop=0, optimize=False)
...
def main():
    ...
    # Create a GIF from the annotated frames, with each frame lasting 1 second.
    if annotated_filenames:
        create_gif(annotated_filenames, gif_name="demo_output2.gif", frame_duration=1.0)
    else:
